...
namespace sdds {
	...
	void FoodOrder::display()const {
		...
		if (m_customer[0] == '\0') {
			std::cout << std::ios::left << std::setw(2) << counter << ". No Order" << std::endl;
		}
		else {
			std::cout << std::ios::left << std::setw(2) << counter << ". " << std::ios::left << std::setw(10) << m_customer << "|" << std::ios::left << std::setw(25) << m_foodDescription << "|" << std::ios::left << std::setw(12) << std::fixed << std::setprecision(2) << priceWTax << "|";
			if (m_dailySpecial == true) {
				std::cout << std::ios::right << std::setw(13) << specialPrice;
			}
			std::cout << std::endl;
		}
